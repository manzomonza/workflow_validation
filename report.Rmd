...
read_input_tsv = function(prep_snvs){
  ...
  if(length(tsvs) == 1){
    ...
    dplyr::select(uid, homopolymer_length:coverage)
  }
  toi$name = base_name
...
for (i in seq_along(path_table_ls)){
  snv_path_1 = as.character(path_table_ls[[3]]$snv_txt[1])
  snv_path_2 = as.character(path_table_ls[[3]]$snv_txt[2])
...
```{r}
# detailed exports
disc = dplyr::left_join(disc, dplyr::bind_rows(tifs))
