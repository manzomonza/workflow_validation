...
create_discordant_igv_report = function(snv_path_1, snv_path_2){
...
add_coordinates = function(res_list){
...
snv_read = function(filepath){
  ...
  dir_path = dirname(dirname(unique(foi$filepath)))
  colnames(foi) = gsub("percent_frequency", base_name, colnames(foi))
  foi$dir_path = dir_path
  return(foi)
}
...
```{r}
complete_table = add_coordinates(complete_list)
discordant_table = add_coordinates(discordant_list)
